...
{
    [BurstCompile]
    public struct HeadingGhostComponentSerializer
    {
        static HeadingGhostComponentSerializer()
        {
            State = new GhostComponentSerializer.State
            {
                GhostFieldsHash = 11266163746584454747,
                ExcludeFromComponentCollectionHash = 0,
                ComponentType = ComponentType.ReadWrite<Heading>(),
                ComponentSize = UnsafeUtility.SizeOf<Heading>(),
        ...
        private static void CopyToSnapshot(IntPtr stateData, IntPtr snapshotData, int snapshotOffset, int snapshotStride, IntPtr componentData, int componentStride, int count)
        {
            for (int i = 0; i < count; ++i)
            {
                ...
                ref var component = ref GhostComponentSerializer.TypeCast<Heading>(componentData, componentStride*i);
        ...
        {
            ref var component = ref GhostComponentSerializer.TypeCast<Heading>(componentData, 0);
            ref var backup = ref GhostComponentSerializer.TypeCast<Heading>(backupData, 0);
        ...
        private static void Serialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamWriter writer, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedFloatDelta(snapshot.Value_x, baseline.Value_x, compressionModel);
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedFloatDelta(snapshot.Value_y, baseline.Value_y, compressionModel);
            if ((changeMask & (1 << 0)) != 0)
                writer.WritePackedFloatDelta(snapshot.Value_z, baseline.Value_z, compressionModel);
        }
        [BurstCompile]
        ...
        private static void Deserialize(IntPtr snapshotData, IntPtr baselineData, ref DataStreamReader reader, ref NetworkCompressionModel compressionModel, IntPtr changeMaskData, int startOffset)
        {
            ...
            uint changeMask = GhostComponentSerializer.CopyFromChangeMask(changeMaskData, startOffset, ChangeMaskBits);
            if ((changeMask & (1 << 0)) != 0)
                snapshot.Value_x = reader.ReadPackedFloatDelta(baseline.Value_x, compressionModel);
            else
                snapshot.Value_x = baseline.Value_x;
            if ((changeMask & (1 << 0)) != 0)
                snapshot.Value_y = reader.ReadPackedFloatDelta(baseline.Value_y, compressionModel);
            else
                snapshot.Value_y = baseline.Value_y;
            if ((changeMask & (1 << 0)) != 0)
                snapshot.Value_z = reader.ReadPackedFloatDelta(baseline.Value_z, compressionModel);
            else
